...
class CurveArea:
    ...
    def _cubic_area(self, plot: bool = True):
        ...
        if plot:
            ...
            yB = m * x + b
            fig, ax = plt.subplots()
            ax.plot(x, y, lw=2, label="Cubic Bézier")
            ax.plot(x, yB, lw=.8, c="k", ls="--", label="Commissure line")
            ax.axvline(x_mid, ls="--", c="k", lw=.8, label="Mid-sagittal")
            # shade cap
            ax.fill_between(x, y, yB, alpha=.25, label="Area")
            ax.set_aspect("equal", "box");
            ax.grid(True)
            ax.set_title(f"Bézier Curve: {self.region}")
            ax.legend();
            plt.show()
