...
class Anchor:
    ...
    Pass either a CSV path or a preloaded DataFrame. Reuse the instance across rows.
    """
...
class Centering:
    def __init__(self, csv_in, row: int, filtering: bool):
        self.csv_in = csv_in
        self.landmarks = pd.read_csv(csv_in)
        self.row = row
    ...
    def center_with_anchor(self, x, y, x_uncertainty: float=None, y_uncertainty: float=None):
        ...
        for col1, col2 in LANDMARK_PAIRS:
            x_val = self.landmarks[col1][self.row]
            y_val = self.landmarks[col2][self.row]
            # print(f"{col1}: {x_val} -- {col1}: {x_val - x} ---------- {col2}: {y_val} -- {col2}: {y_val - y}")
